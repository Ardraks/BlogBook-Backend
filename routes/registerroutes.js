...
app.post('/registernew', async (request, response) => {
  
  const { username, email, password,confirmPassword,status} = request.body;
  try
  {
      const newUser = new Registermodel({ username, email, password,confirmPassword,status });
      await newUser.save();
